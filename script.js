...
function loadPositionTTL(key, rawLength) {
  try {
    ...
    if (!d || Date.now()-d.savedAt>CACHE_TTL || d.videos.length!==rawLength) {
      localStorage.removeItem(`pls_pos_${key}`); return null;
    }
    return d;
  } catch(e) { return null; }
}
function loadPositionPermanent(key, rawLength) {
  try {
    const d = JSON.parse(localStorage.getItem(`pls_pos_${key}`)||'null');
    if (!d || d.videos.length!==rawLength) { localStorage.removeItem(`pls_pos_${key}`); return null; }
    return d;
  } catch(e) { return null; }
}
function saveState() {
...
function nextCombo() {
  if (comboPool.length === 0) {
    if (firstPool) {
      firstPool = false;
      do { comboPool = shuffle([...COMBO_WEIGHT_POOL]); }
...
function buildScheduleCards() {
  ...
  SLOTS.forEach((s,i) => {
    ...
    d.className='scard'; d.id=`scard-${i}`;
...
function updateActiveCard() {
  const a = resolveSlot();
  SLOTS.forEach((s,i)=>document.getElementById(`scard-${i}`)?.classList.toggle('is-active',s===a));
}
...
function scrambleText(el, text, dur) {
  return new Promise(resolve => {
    ...
    function frame(now) {
      const p=Math.min((now-t0)/dur,1), n=Math.floor(p*len);
      let s='';
      for(let i=0;i<len;i++) {
        if(text[i]===' '){s+=' ';continue;}
        s+=i<n?text[i]:SCRAMBLE_CHARS[Math.floor(Math.random()*SCRAMBLE_CHARS.length)];
      }
      el.textContent=s;
...
function playVideo(plId) {
  ...
  if (!vids.length) { console.error(`No videos for "${plId}"`); return; }
...
async function playNext() {
  ...
  try {
    if (pendingSlotSwitch) {
      ...
      if (!newSlot) { currentSlotKey=null; return; }
...
async function loadAllPlaylistsAndStart(slot) {
  ...
  if (saved && saved.slotKey === currentSlotKey) {
    ...
    if (saved.mode === 'commercial' && saved.comboIndex >= 0) {
      ...
      console.log(`Restoring: commercial combo ${saved.comboIndex+1} step ${comboStep}`);
      playCurrentComboStep();
      return;
    }
  }
